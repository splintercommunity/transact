...
const FORCE_PANDOC: &str = "TRANSACT_FORCE_PANDOC";
const PATH: &str = "PATH";
...
/// generated set environment variable TRANSACT_FORCE_PANDOC=true
    ...
    if !pandoc_exist {
        if let Ok(var) = env::var(FORCE_PANDOC) {
            let map_to_build_err = move |_| {
                BuildError("Unable to read TRANSACT_FORCE_PANDOC environment variable".into())
            };
